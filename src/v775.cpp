...
int v775::read_single_evt(int am, uint32_t *evt, int* sz_out)
{
	int ret, am_old;
	...
	*sz_out = 0;
	do {
		/* event buffer address (offset) in the v775 is 0x0000 */
 ...
 * address (0xBB)
 * @return 0 if succeed, nonzero error codes if error. */
int v775::enable_mcst(uint32_t mcst_addr)
{
	return -E_NOT_IMPLE;
}
 ...
 * chain
 * @return 0 if succeed, nonzero error codes if error.*/
int v775::enable_cblt(uint32_t cblt_addr, int first, int last)
{
	return -E_NOT_IMPLE;
}
int v775::get_cblt_conf(uint16_t* addr, int* cblt_enable, int* cblt_first,
		int* cblt_last)
{
	int ret;
	uint32_t val;
