...
int mqdc32::read_single_evt(int am, uint32_t *evt, int* sz_out)
{
	int ret, am_old;
	...
	*sz_out = 0;
	do {
		/* event buffer address (offset) in the mqdc32 is 0x0000 */
 ...
 * address (0xBB)
 * @return 0 if succeed, nonzero error codes if error. */
int mqdc32::enable_mcst(uint32_t mcst_addr)
{
	int ret;
	uint16_t val;
	if (mcst_addr) {
 ...
 * chain
 * @return 0 if succeed, nonzero error codes if error.*/
int mqdc32::enable_cblt(uint32_t cblt_addr, int first, int last)
{
	int ret;
	uint16_t val;
...
int mqdc32::get_cblt_conf(uint16_t* addr, int* cblt_enable, int* cblt_first,
		int* cblt_last)
{
	int ret;
	uint16_t val;
