...
{
	uint32_t sig;
	uint32_t buf[50]; /* big enough to accomadate a mqdc32 event plus the
			     additional header .*/
...
err_data:
	...
	send_text_mes("corrupted mqdc32 data", MSG_LEV_WARN);
	return 0;
//	return -E_DATA_MADC32;
}
...
int ebd_sort::handle_single_evt_mdpp(uint32_t* evt, int& evt_len, int max_len)
{
	uint32_t sig;
	uint32_t buf[150]; /* big enough to accomadate a mdpp event plus the
			     additional header .*/
	uint64_t ts, ts_hi, ts_mono, clk_freq, evt_cnt, clk_off;
	bool has_et = false;
	int idx, evt_len_w;
...
err_data:
	 ...
	 * return it in the param 'evt_len'*/
	evt_len = max_len*4; //abonden the whole block of data.
	send_text_mes("corrupted mdpp data", MSG_LEV_WARN);
	return 0;
//	return -E_DATA_MADC32;
}
