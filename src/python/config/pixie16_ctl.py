...
__metaclass__ = type
 ...
 # This is the pixie16_ctl module class (modified based on the vme_mod.py). The
 # pixie16_ctl module object maintains all configuration data for the module.
 # The pixie16_ctl module is, unlike vme controllers, not physically existing.
 # But we still need some parameters when comunicate the pixie16 modules and
 # those parameters are managed by this module. The pixie16_ctl module object
 ...
 # configurations.  Similarly as the vme_mod, I also use the variable 'reg_map'
 # for the centrol data structure of the class. It is a list, each element is a
 # dict which has the following structure. (note, the elements in this dict
 # doesn't have to be a physical register of the module, it can be any setting
 # parameter with uniq offset. In the case of pixie16_ctl module, most, if not
 # all, of the setting parameters are not physical registers.):
 #  {'off':offset, ---> offset of the register
 #   'value':value, ---> value of the registter, unlike vme_mod, the special
 #                       value 'default' is *NOT* allowed.
