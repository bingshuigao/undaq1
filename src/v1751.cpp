...
int v1751::read_single_evt(int am, uint32_t *evt, int* sz_out)
{
	...
	*sz_out = 4;
	...
	while (--len_dw) {
		evt++;
		/* event buffer address (offset) in the v1751 is 0x0000 */
		ret = read_reg(base_addr, 32, (void*) evt);
		if (ret) 
			goto end;
		*sz_out += 4;
 ...
 * address (0xBB)
 * @return 0 if succeed, nonzero error codes if error. */
int v1751::enable_mcst(uint32_t mcst_addr)
{
	return -E_NOT_IMPLE;
}
 ...
 * chain
 * @return 0 if succeed, nonzero error codes if error.*/
int v1751::enable_cblt(uint32_t cblt_addr, int first, int last)
{
	return -E_NOT_IMPLE;
}
int v1751::get_cblt_conf(uint16_t* addr, int* cblt_enable, int* cblt_first,
		int* cblt_last)
{
	int ret;
	uint32_t val;
